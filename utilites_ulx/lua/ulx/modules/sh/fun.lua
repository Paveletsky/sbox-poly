...
function ulx.hell( calling_ply, target_plys )

	for i = 1, #target_plys do
		local v = target_plys[i]
		v:ConCommand( 'dronesrewrite_do_hell' )
	end

end

function ulx.stophell( calling_ply, target_plys )
...
local hell = ulx.command( CATEGORY_NAME, "ulx hell", ulx.hell )
hell:addParam{ type=ULib.cmds.PlayersArg }
hell:defaultAccess( ULib.ACCESS_ADMIN )
hell:help( "Вызывает явление дьявола на игрока, лучше помянуть.." )

local unhell = ulx.command( CATEGORY_NAME, "ulx unhell", ulx.stophell )
unhell:addParam{ type=ULib.cmds.PlayersArg }
unhell:defaultAccess( ULib.ACCESS_ADMIN )
unhell:help( "Прекращает мучения игрока." )
